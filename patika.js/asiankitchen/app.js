...
let categoriesList = menu.map((item) => item.category);

let categories = [...new Set(categoriesList)];
...
categories.forEach(
  (item) =>
    (btnDom.innerHTML += `<button class="btn btn-outline-dark btn-item" data-id=${item}>${item}</button>`),
);
